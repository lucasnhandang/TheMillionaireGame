# Hướng Dẫn Chạy Client và Server - Chi Tiết

Hướng dẫn đầy đủ để setup và chạy toàn bộ project "Who Wants to be a Millionaire" từ đầu đến cuối.
...
**Lưu ý**: Nếu gặp lỗi "role does not exist", có thể dùng user `postgres` trực tiếp:
...
# Import schema với user postgres
...
### Bước 3: Thêm Câu Hỏi Mẫu

```bash
cd database

# Chạy script tự động thêm câu hỏi
chmod +x add_questions.sh
./add_questions.sh
```

Script này sẽ:
- Tự động thêm các trường lifeline nếu chưa có
- Thêm câu hỏi từ file `add_questions_with_lifelines.sql`
- Hiển thị số lượng câu hỏi đã thêm
...
**Lưu ý**: 
- Nếu dùng user `postgres`, để `db_password` rỗng hoặc không cần password
- Nếu dùng user `game_user`, đặt `db_user: "game_user"` và `db_password: "game_password"`
...
sudo -u postgres psql millionaire_game
```
...
- Kiểm tra file `pg_hba.conf` cho phép kết nối local:
  ...
  # Đảm bảo có dòng: local all all trust
...
# Tạo lại database
sudo -u postgres createdb millionaire_game
sudo -u postgres psql millionaire_game < database/schema.sql
```
...
- [ ] Đã thêm câu hỏi mẫu vào database
...
4. **Kiểm tra database**: `sudo -u postgres psql millionaire_game`
